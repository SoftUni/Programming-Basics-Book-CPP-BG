...
В горния пример караме компютъра да отпечатва един след друг много символи, започвайки от А и приключвайки с Z. Резултатът от програмата е отпечатване на цялата английска азбука.
...
В **`int main()`** метода пишем 20 команди **``std::cout << ... << std::endl;``**, всяка на отделен ред, за да отпечатаме числата от 1 до 20 едно след друго. По-досетливите от вас, сигурно се питат дали няма по-умен начин. Спокойно, има, но за него по-късно.
