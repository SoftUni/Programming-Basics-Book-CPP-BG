...
Също така за сега ще пишем командите на C++ в **`int main()`**, между отварящата и затварящата скоба **`{ }`**. Това е главният метод (действие), което се изпълнява при стартиране на една C++ програма. По-нататък в книгата в главата **[Функции](chapter-10-functions.md)"** ще се научим къде другаде и как можем да пишем код. 
