...
```
Можем да тествате примера онлайн: https://repl.it/@nakov/CPlusPlus-Hello.
...
Слагаме отметка на [**Desktop development with C++**].
...
Когато сме готови до тук, ще се отвори прозорец, който ще изглежда като следващата картинка, и трябва да изберем [**Visual C++**] &rarr; [**Empty Project**]. След което задаваме **смислено име** на нашия проект, например `HelloC++`. 
...
За да добавим **празна C++ програма** трябва да изберем: [**Project**] &rarr; [**Add new item**]:
![](/assets/chapter-1-images/01.Hello-c++-04.png)

Ще се отвори друг прозорец, в който трябва да изберем [**Visual C++**] &rarr; [**C++ file(.cpp)**]. След което задаваме **смислено име** на нашата програма, например `HelloC++.cpp`. Ако забравим да напишем **.cpp**, Visual Studio ще го добави:
...
Командата **`cout << "Hello C++" << endl;`** означава да изпълним отпечатване върху конзолата и да отпечатаме текстовото съобщение **`Hello C++`**, което трябва да оградим с кавички, за да поясним, че това е текст. **`endl`** означава нов ред и **`cout << endl;`** означава отпечатване на нов ред. Тези команди могат да се напишат и на два реда:
```cpp
cout << "Hello C++";
cout << endl;
```
...
Редът **#include<iostream>** означава включи кода от библиотеката **iostream**. **include** буквално означава включи този код в моя. Библиотеките са код, който можем да използваме на готово. Например от библиотеката **iostream** използваме **cin** и **cout**. Има най-различни други библиотеки, също така сами можем да си пишем библиотеки, но няма да се занимаваме с това сега.

Редът **using namespace std;** означава използвай namespace-a **std**. Какво е namespace? Представете си, че по даден проект работят много хора, сред които Пешо и Гошо. Пешо иска да си кръсти някоя променлива **broi**, но и Гошо иска да използва променлива с име **broi**. За компилатора тези две променливи ще означават едно и също, независимо дали Пешо или Гошо я използва. За да се реши този проблем, се използва **namespace**. Тогава Пешо ще си има namespace Pesho, а Гошо ще си има namespace Gosho. За да използва Пешо своята променлива **broi**, ще напише **Pesho::broi**, а Гошо, за да използва своята променлива **broi**, ще напише съответно **Gosho::broi**. Тогава вече компилаторът ще прави разлика между променливата **broi** на Гошо и съответно на Пешо. Символите **::** означават да се вземе променливата, написана отдясно на **::**, от namespace-а, написан отляво на **::**. 

Има и готови namespace-ове, като например **std**. Съкращението **std** идва от **Standard Template Library** и конкретно от думата **standart**. Някои библиотеки са обединени в namespace-a **std**. Т.е. за да използваме нещата от тях, трябва да пишем навсякъде **std::...**. Както вече казахме, редът **using namespace std;** означава използвай namespace-a std. Т.е. ако го напишем в програмата си, включваме всички неща от съответния namespace и няма нужда навсякъде да пишем **std::...**. Горният код без този ред ще изглежда така:

![](/assets/chapter-1-images/01.Hello-c++-09.png)

В книгата ще използваме първия вариант с **using namespace std;**.
...
Забележете, че стартираме с **[Ctrl+F5]**, а не само с **[F5]** или с бутона за стартиране във Visual Studio. Ако използваме **[F5]**, програмата ще се изпълни за кратко и веднага след това черният прозорец ще изчезне и няма да видим резултата.
...
Възможно е **[Ctrl+F5]** да не работи и черният прозорец да изчезне веднага. Това, което се случва, е, че програмата се изпълнява и черният прозорец се затваря веднага след изпълнението й, заради това не виждаме резултата. За да оправим проблема, трябва да добавим следния ред код на реда **преди return 0**:
