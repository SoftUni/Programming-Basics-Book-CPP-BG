...
* За езика C++ можем да използваме: [http://cpp.sh/]
...
Задаваме **смислено име** на нашата програма, например `HelloC++`:

![](/assets/chapter-1-images/01.Hello-csharp-02.png)

Visual Studio ще създаде за нас **празна C++ програма**, която трябва да допишем (VS Solution с VS Project в него със C++ сорс файл в него).

### Писане на програмен код

За сега ще пишем командите на C++ в **`int main()`**, между отварящата и затварящата скоба **`{ }`**. Това е главният метод (действие), което се изпълнява при стартиране на една C++ програма. Този главен **`main()`** метод се записва така:
 - **`int main()`**

По-нататък в книгата в главата **главата "[Функции](chapter-10-functions.md)"** ще се научим къде другаде и как можем да пишем код. 
...
Командата **`std::cout << "Hello C++" << std::endl;`** означава да изпълним отпечатване върху конзолата и да отпечатаме текстово съобщение **`Hello C++`**, което трябва да оградим с кавички, за да поясним, че това е текст. **`std::endl`** означава нов ред и **`std::cout << endl;`** означава отпечатване на нов ред. Тези команди могат да се напишат и на два реда:
```cpp
std::cout << "Hello C++";
std::cout << std::endl;
```
...
Във Visual Studio ще се изпише следната грешка: **error C2146: syntax error : missing ';' before identifier 'cout'**
...
Във Visual Studio ще се изпише следната грешка: **'x' : undeclared identifier**
...
Тази програма ще даде **грешка при опит за компилация** и стартиране и даже още преди това кодът ще бъде подчертан, за да се насочи вниманието на програмиста към грешката, която е допуснал (пропуснатата отваряща скоба).


## Какво научихме от тази глава?

На първо място научихме **какво е програмирането** - **задаване на команди, изписани на компютърен език**, които машината разбира и може да изпълни. Разбрахме още какво е **компютърната програма** - тя представлява **поредица от команди**, подредени една след друга. Запознахме се с **езика за програмиране C++** на базисно ниво и как **да създаваме прости конзолни програми** с Visual Studio. Проследихме и **структурата на програмния код в езика C++**, като например, че за сега ще задаваме командите в секцията **``int main()``** между **отварящата и затварящата къдрава скоба**. Видяхме как да печатаме с **`std::cout << ... ;`** и как да стартираме програмата си с [**Ctrl + F5**]. Научихме се да тестваме кода си в **SoftUni Judge**.
...
Правим **нов C++ конзолен проект** с име "**Expression**".	Намираме метода **``int main()``** и **влизаме в неговото тяло** между **`{`** и **`}`**. След това трябва да **напишем кода**, който да изчисли горния числен израз и да отпечата на конзолата стойността му. Пишем горния числов на мястото на многоточието в следната команда: [**``std::cout << ... ;``**:
...
Създаваме **конзолно C++ приложение** с име “**Nums1To20**“. В **`int main()`** метода пишем 20 команди **``std::cout << ... << std::endl;``**, всяка на отделен ред, за да отпечатаме числата от 1 до 20 едно след друго:
```cpp
std::cout << 1 << std::endl;
std::cout << 2 << std::endl;
…
...
Остава да се допише програмата по-горе, за да пресмята лицето на правоъгълника и да го отпечата. Използвайте познатата ни вече команда **`std::cout << ... ;`** и на мястото на многоточието напишете произведението на числата **a** и **b**. В програмирането умножението се извършва с оператора **`*`**.
