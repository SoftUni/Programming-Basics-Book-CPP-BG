...
В **`int main()`** метода пишем 20 команди **``std::cout << ... << std::endl;``**, всяка на отделен ред, за да отпечатаме числата от 1 до 20 едно след друго. Командата **``std::cout << std::endl;``** извежда нов ред. Можем да напишем командите на извеждане на число и нов ред след него така:
```cpp
std::cout << 1;
std::cout << std::endl;
std::cout << 2;
...
Или пък на един ред по този начин:
```cpp
std::cout << 1 << std::endl;
std::cout << 2 << std::endl;
…
