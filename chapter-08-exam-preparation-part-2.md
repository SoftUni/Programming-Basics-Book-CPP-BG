...
Напишете програма, която да пресмята **колко километра изминава кола**, за която знаем **първоначалната скорост** \(км/ч\), **времето** в минути, след което **увеличава скоростта с 10%**, **второ време**, след което **намалява скоростта с 5%**, и **времето до края** на пътуването. За да намерите разстоянието трябва да **превърнете минутите в часове** \(например 70 минути = 1.1667 часа\).
...
За **прочитането** на данните от **конзолата** използваме **cin**:
...
[TODO: Линк за различните типове в C++]
...
<tr>
<td width=10%><img src="/assets/alert-icon.png" style="max-width:50px" /></td>
    <td>Чрез запазената дума <code>const</code> поставена пред типа <code>double</code> показваме, че променливата <code>minutesPerHour</code> е <strong>константа</strong>, т. е. не може да бъде променяна. Веднъж запазена стойността <code>60.0</code> в променливата, тя <strong>не може да бъде променяна</strong>. На пръв поглед използването на константи в задачите, които разглеждаме в настоящата книга, също изглежда излишно, но те също имат своето приложение в мащабните проекти. Те могат да предотвратят създаването на бъдещи грешки при разработката на софтуер, защото ако компилатора забележи опит за промяна на константа, той веднага ще даде грешка по време на компилация. Също, в зависимост от компилатора, използването на константи може леко да ускори изпълнението на програмата и/или леко да намали използваната памет от програмата.
</td>
</tr>
</table>
...
<tr>
<td width="10%"><img src="/assets/alert-icon.png" style="max-width:50px" /></td>
<td>Обърнете внимание, че в условието е упоменато да закръглим броя на плочките, получен от делението, до по-високо цяло число, след което да прибавим 5. Потърсете повече информация за системната функционалност за това: <code><strong>ceil(…)</strong></code>.
</td>
</tr>
</table>
...
Следващата ни подзадача е да **прочетем** правилно **входните** данни от конзолата:
