...
Не забравяйте да добавите библиотеката **`<string>`**, когато искате да въвеждате и извеждате текст.
...
Тествайте решението си тук:  [https://judge.softuni.bg/Contests/Practice/Index/1363#0](https://judge.softuni.bg/Contests/Practice/Index/1363#0).
...
Както вече разбрахме, е **добра практика** да **разделяме кода си на части**, но при тази задача това само би я **усложнило**.
...
Следователно първо може да проверим дали второто число е **`0`** (нула), както и дали операцията е **деление** или **модулно деление**, след което да изпринтим резултата. 
...
В **C++** не можем да използваме оператора за **модулно деление** (**`%`**) с реални числа, затова ще трябва да използваме функцията за модулно деление **`fmod(делимо, делител)`**, а за нея ще ни е необходима и библиотеката **`<cmath>`**.
...
Нека използваме **`if-else`** условна конструкция.
...
![](/assets/chapter-4-2-images/04.Match-tickets-07.png)
