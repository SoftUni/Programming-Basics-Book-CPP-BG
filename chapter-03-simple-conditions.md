...
Закръглете изходната стойност, подавайки последователно към **`cout`** функциите **`std::fixed`** и **`std::setprecision(n)`**, където с **n** указвате до кой знак искате да закръглите. Включете и следният файл в началото на вашата програма:

```cplusplus
#include <iomanip>
```

за да можете да използвате **`std::setprecision()`**. Ето примерно закръгляне:
...
при което като изход ще бъде изведено **3.14**.
...
Да се напише програма, която превръща число в диапазона [**0 … 100**] в текст. Ако числото е извън този диапазон, да изписва **invalid number**.
