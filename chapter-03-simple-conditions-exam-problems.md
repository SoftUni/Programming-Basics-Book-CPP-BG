...
След като вече сме **декларирали и инициализирали** входните данни и променливата, в която ще пазим стойността на цената, трябва да преценим кои **условия** от задачата да бъдат първи проверени. 
...
Тествайте решението си тук: [https://judge.softuni.org/Contests/Practice/Index/1361#0](https://judge.softuni.org/Contests/Practice/Index/1361#0).
...
Това ще направим с проста **`if-else`** конструкция, в която условието ще проверява дали **количеството вода е по-малко от обема на басейна**. Ако проверката върне **`true`**, то трябва да разпечатаме един **ред**, който да съдържа **съотношението** между количеството **вода, минало през тръбите**, и **обема на басейна**, както и **съотношението на количеството вода** от **всяка една тръба** спрямо **обема на басейна**. 

Съотношението е нужно да бъде изразено в **проценти**, затова и всички изчисления до момента ще бъдат **умножени по 100**. Тъй като има условие **резултата в проценти** да се форматира до **две цифри** след **десетичния** знак без закръгляне, то за целта ще използваме функцията **`trunc(…)`**:
...
Тествайте решението си тук: [https://judge.softuni.org/Contests/Practice/Index/1361#1](https://judge.softuni.org/Contests/Practice/Index/1361#1).
...
Тук трябва да отбележим, че ако полученото количество **време игра** на Том е **по-малко** от **30 000**, при **изваждането** на нормата от него ще получим **число с отрицателен знак**. За да **неутрализираме** знака в двете деления по-късно, ще използваме **функцията `abs(…)`** при намирането на разликата:
...
Тествайте решението си тук: [https://judge.softuni.org/Contests/Practice/Index/1361#2](https://judge.softuni.org/Contests/Practice/Index/1361#2).
...
Тествайте решението си тук: [https://judge.softuni.org/Contests/Practice/Index/1361#3](https://judge.softuni.org/Contests/Practice/Index/1361#3).
...
Тествайте решението си тук: [https://judge.softuni.org/Contests/Practice/Index/1361#4](https://judge.softuni.org/Contests/Practice/Index/1361#4).
