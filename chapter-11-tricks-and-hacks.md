...
Езикът C++ е направен от **Бьорн Строуструп** (Bjarne Stroustrup - https://en.wikipedia.org/wiki/Bjarne_Stroustrup). Трябва да знаете, че дори да не спазвате конвенциите, кодът ви ще работи (стига да е написан правилно), но просто няма да бъде лесно разбираем. Това, разбира се, не е фатално на основно ниво, но колкото по-бързо свикнете да пишете качествен код, толкова по-добре.

**C++ код конвенция** на Bjarne Stroustrup е публикувана в статията "Bjarne Stroustrup's C++ Style and Technique FAQ" в (http://www.stroustrup.com/bs_faq2.html) и в тази книга ще се ръководим основно от нея.

За форматиране на кода се препоръчва **къдравите скоби `{}`** да са по следния начин. Отварящата скоба `{` да е на същия ред и точно след конструкцията, към която се отнася. Затварящата скоба `}` се намира самостоятелно на последния ред от конструкцията, както е в примера по-долу.
...
Вижда се, че командата **`cout`** в примера е **4 празни полета навътре (един таб)**, което също се препоръчва от **Microsoft**. Също така, ако дадена конструкция с къдрави скоби е един таб навътре, то **къдравите скоби `{}`** трябва да са в **началото на конструкцията**, както е в примера по-долу:
...
Първото, което се забелязва са **къдравите скоби `{}`**. Първата (отваряща) скоба трябва да е **точно под `if` условието**, a втората (затваряща) скоба - **под командата `cout << (…)`, на отделен празен ред**. В допълнение, командата вътре в **`if`** конструкцията трябва да бъде **4 празни полета навътре (един таб)**. Веднага след ключовата дума **`if`** и преди условието на проверката се оставя **интервал**.
...
Добрата практика е променливи с изцяло главни букви да се ползват само за имена на **Macros** https://docs.microsoft.com/en-us/cpp/preprocessor/macros-c-cpp?view=vs-2017</td>
</tr></table>
...
Във Visual Studio съществуват т.нар. **шаблони с код** (code snippets), при изписването на които се изписва по шаблон някакъв блок с код. Примерно, при изписването на кратък код “**`for`**” и натискане на **[Tab]** се генерира следният код
```cpp
...
```
в тялото на нашата програма, на мястото на краткия код. Това се нарича “разгъване на шаблон за кратък код”. На фигурата по-долу е показано действието на шаблона “**`for`**”:

![](/assets/chapter-11-images/01.Code-snippet-01.jpg)

### Шаблони за код

Можем да си **направим собствен шаблон** или да свалим от интернет и да добавим(**add**) или вмъкнем(**import**). Това се прави от [**Tools -> Code Snippets Manager**], както е показано на снимката:
...
В този пример забелязваме, че можем да подаваме **не само текстови променливи**. **`%s`** указва типа на следващата **`%string`** прменлива. **`%d`** се използва са числов тип **`int`**.
...
Закръглете резултата до **2 цифри след десетичния знак използвайки `fixed << setprecision(2)`**, като включите библиотеката **`<iomanip>`**
...
В случай, че искаме винаги да закръгляме надолу можем вместо да използваме функцията **`floor(…)`**, която винаги закръгля надолу, но също така винаги закръгля до цяло число. Например, ако имаме числото 5.99 и използваме **`floor(5.99)`**, ще получим числото **5**.

Можем и да направим точно обратното - винаги да закръгляме нагоре, използвайки метода **`ceil(…)`**. Отново, ако имаме число примерно 5.11 и използваме **`ceil(5.11)`**, ще получим **6**. Ето и няколко примера:
...
В случая след числото добавяме **`.2f`**, което ще ограничи числото до 2 цифри след десетичния знак и ще работи като **`setprecision(…)`**. Трябва да имаме предвид, че числото преди буквата **`f`** означава до колко цифри след десетичния знак да е закръглено числото (т.е. може да е примерно **`3f`** или **`5f`**).
