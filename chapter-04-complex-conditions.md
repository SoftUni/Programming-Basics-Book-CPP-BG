...
да изпълни различни инструкции, в зависимост от стойността на променлива или
израз.
Тази променлива или израз се нарича **селектор** и се поставя в **скобите след
оператора `switch`**.
Заградени в къдрави скоби следват **`case:` етикети**, чрез които обозначаваме
какъв код да се изпълни при различните стойности на селектора.
...
са **от целочислен тип** (например **`int`**).
За разлика от други езици за програмиране, C++ **не позволява** използването на
конструкция **`switch-case`** с променливи от **текстов тип** (**`string`**).

Ако съществува **case: етикет** равен на стойността на **селектора**,
**switch-case** започва да изпълнява кода след съответния етикет.
В противен случай започва да се изпълнява кодът след специалния етикет
**`default`**, ако има такъв.
Изпълнението на кода продължава до срещането на оператора **break;**.
Практически е **задължително** да се използва **break;**, тъй като в противен
случай, може да се изпълни кодът, предвиден за повече от един етикет.

**`Switch-case`** може винаги да бъде заместен чрез последователност от
**`if-else`** конструкции – общият код за **`switch-case`**, е еквивалентен на
следния код, използващ **`if-else`** конструкции:
...
Кодът, иползващ **switch-case**, обаче е за предпочитане, тъй като е по-кратък и
ясен.
Освен това, тъй като **switch-case** е ограничен, до целочислени стойности,
много компилатори го реализират по по-бърз начин от последователни **if-else**
конструкции.
