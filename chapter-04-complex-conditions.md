...
(if), в C++ можем да запазим резултата в променлива, която да ползваме по-късно.
...
### Операторът &&
Операторът && се прилага две булеви стойности и на свой ред && също връща
булева стойност.
Когато и двата аргумента имат стойност true, стойността на израза също е true,
а в противен случай тя е false.
Аргументи могат да бъдат както променливи от булев тип, така и други резултатите
от други проверки.

Долният пример показва как можем да използваме операторът &&, за да проверим
дали въведено число е между 100 и 200:
...
Един оператор + събира директно само две числа.
Затова, за да съберем повече от две числа, трябва да използваме няколко
оператора +:
```cpp
int sum = 1 + 2 + 3 + 4;
```

Чрез && можем по аналогичен начин да проверяваме дали повече от две
условия са условия са изпълнени едновременно:
...
Подобно на обикновенните аритметични операции (събиране, умножение, и. т.н.),
логическото "и" (&&) се изчислява от ляво надясно. 
За да има изразът стойност true, не може да има и един аргумент със стойност
false.
C++ се възползва от това, като спира да проверява аргументите на && при първото
срещане на стойност false (от ляво надясно), тъй като това директно означава, че
стойността на целия израз също е false.
В горния пример, условие d няма да се провери, тъй като 
<!---
искаме пример с деление на нула, иначе няма смисъл да е обяснява short circuit
--->
