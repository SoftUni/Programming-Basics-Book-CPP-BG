...
* **Имплеметацията (тялото)**. Записва се в **областта**, отделена от скобите **`{`** и **`}`**. Тази област се създава **след** списъка с параметри (вж. последния дотук програмен фрагмент).
