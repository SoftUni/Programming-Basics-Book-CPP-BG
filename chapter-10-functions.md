...
**Заради известни съображения**, на които **няма сега да се спираме**, ще дефинираме **още един елемент, част от метода за въвеждане на стринг с интервали**, а именно **`cin.ignore();`**. Този ред е **необходимо да бъде изписан над** **`getline(cin, str);`**. Така постъпваме, когато **преди стринга сме въвели стойности от друг тип данни** (**`int`**, **`char`** и други). Ако последователно се въвеждат стрингове с интервали, то между тях не се поставя редът, който споменахме. Ще ралгледаме откъс от проглама, въвещдаща различни сътойности, сред които три стринга, в състава на които може да има интервал (space):
