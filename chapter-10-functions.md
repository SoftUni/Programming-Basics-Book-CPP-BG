...
<img src = "/assets/chapter-10-images/01_simple_function_01.PNG"/>
...
**Тялото** на функцията, което се намира между кърдравите скоби **`{`** и **`}`**, съдържа **програмния код** (инструкциите), който решава проблема, описан от името на функцията. Тялото на функцията се изписва по-навътре, обикновено 4 интервала (една табулация), които го обособяват като отделен блок инструкции, прилежащи към функцията.  
...
**Важно:** Разгледаните тук функции не връщат стойност (резултат). Тях ще разгледаме малко по-късно, в секция [**Връщане на резултат от функция**](#връщане-на-резултат-от-функция).
...
* Декларираме функцията **`printSentence()`** преди .
...
// Декрарираме и дефинираме функцията едновременно
void printSentence() {                     
...
Оказва се, че резултатът от двете програми е **еднакъв**. Разликата е само, че при втория подход **декларирането и дефинирането** на функцията са слeти в едно, като това се случва преди главната функция **`main()`**. Факт е, че вторият подход изисква по-малко писане на код, но в **практиката** често се използва **първия подход**, тъй като дава по-добра **четимост** на програмата, както и поради **други причини**. 
...
<table><tr><td><img src="/assets/alert-icon.png" style="max-width:50px" /></td>
 <td>Оттук нататък в главата даваните примери ще бъдат <b>само дефиниции</b> на функции. Оставяме читателя <b>сам</b> да създава техните <b>прототипи</b>.</b></td>
</tr></table>
...
Нека обърнем внимание, че за да се отпечата символът "\\", трябва да се въведе в програмата "\\\\".
Накрая извикваме създадените функции в главния **`main()`** функция на програмата, за да нарисуваме целия квадрат:
...
</td><td>За да върне една функция <strong>резултат</strong> е нужно да напишем очаквания тип на резултата при декларацията на функцията на мястото на <code>void</code>.</td></tr>
</table>
...
След оператора **`return`**, в текущия блок, **не** трябва да има други редове код, тъй като изпълнението на функцията се прекратява и продължава на мястото, от където е извикана функцията. Ако след оператора **`return`** има други инструкции, то те няма да бъдат изпълнени. Някой редактори, в това число и Visual Studio, ще покажат предупреждение, съобщавайки ни, че е засечен код, който **не може да бъде достъпен**:
...
Първо създаваме функция, която да изчислява лицето на базата на две променливи - дължината на страната **length** и височината **height**. Този път внимаваме при **декларацията** да подадем коректен **тип** данни, които искаме функцията да върне, а именно **`float`** (може да бъде и **`double`**):
...
![](/assets/chapter-10-images/19_compare_03.PNG)
...
**Важно** е да се отбележи, че един въвеждан стринг не е задължително да бъде само във вид на **"дума"**, т.е. един стринг може да представлява набор от няколко "думи" - казано по друг начин, **допустимо е при въвеждането му да се използва space (интервал) като част от структурата му** (например низът **`this is a string`**). За да може обаче това да бъде изпълнено в дадена програма, се **изисква** стрингът да бъде **въведен по по-различен начин**, а именно:

```cpp
string str;
getline(cin, str);
```
**За да можем да въвеждаме по такъв начин**, **трябва предварително да сме декларирали** библиотеката **string** (в зависимост от средата за програмиране може да бъде срещната и като **cstring** и дори **string.h**) - **`#include <string>`**.

**Заради известни съображения**, на които **няма сега да се спираме**, ще дефинираме **още един елемент, част от метода за въвеждане на стринг с интервали**, а именно **`cin.ignore();`**. Този ред е **необходимо да бъде изписан над** **`getline(cin, str);`**. Така постъпваме, когато **преди стринга сме въвели стойности от друг тип данни** (**`int`**, **`char`** и други). Ако **последователно** се въвеждат **стрингове с интервали**, то между тях **не се поставя** редът, който споменахме. Ще ралгледама проглама, въвеждащата и отпечатваща различни стойности, сред които три стринга, като в състава им може да има интервал (space):
...
int main() {
    ...
    char symbol;
    string strNormal;
    string strSpace1, strSpace2, strSpace3;
    cin >> number;
    
    cin.ignore();
    getline(cin, strSpace1);
    getline(cin, strSpace2);
    cin >> symbol;
    cin >> str;
    
    cin.ignore();
    cin.getline(cin, strSpace3);
...
```
Низовете, в които има интервали, се сравняват **лексикографски**, както познатите ни дотук стрингове. Въвеждането на стринг по този начин е **полезно умение**, даващо по-голяма **свобода** при работа с низове, както и **практичност**.
#### Тестване в Judge системата
...
При решаването на тази задача се достига до това, че трябва променлива то тип данни **`int`** да се **превърне** в **`string`**. Това може да стане по начина, описан отдолу. Засега **няма да обясняваме** защо точно така се прави, а просто ще го **приемем** наготово. Този метод ще използваме **не само** за да конвертираме **`int`**, но и **други** типове данни (**`char`**, **`double`** и други) в **`string`**. Нека обърнем внимание на това, че е необходимо да бъде декларирана библиотеката **sstream** (**`#include <sstream>`**), както и библиотеката **string** (**`#include <string>`**).

Въпреки че **не можем да си обясним** как реално се случват нещата в предложената по-долу програма, базирайки се само на **досегашните си знания**, то може да се опитаме поне да изградим някаква **представа**, която да е **ориентировъчна**, но **не и абсолютно коректна**. Нека сме въвели целочислената стойност **`number`** (вж. примера) (излишно е да припомняме, че сме декларирали необходимите библиотеки, нужни на програмата). Нека чрез думата **stringstream** декларираме **`numstr`**, което съвсем условно ще наречем **файл**. По-нататък чрез знака **"<<"** сякаш **запълваме "файла"** със **стойността на** **`number`**. Накрая **декларираме стринг** по стандартен начин (в случая той е **`newstr1`**. Стойността, която му задаваме да присвои, е всъщност тази на "файла" **`numstr`**. Единствената **особеност** е, че при присвояването след **`numstr`** се **поставя** знакът "точка" (**`.`**) и **`str()`**. **Краят на присвояването** отговаря на общото **правило**, според което се поставя **"точка и запетая"** (**`;`**). За да се ориентираме по-добре, е хубаво всичко казано дотук да **проследим** в предложения код.
