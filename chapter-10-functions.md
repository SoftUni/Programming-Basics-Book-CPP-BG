...
* **Имплеметацията (тялото)**. Записва се в **областта**, отделена от скобите **`{`** и **`}`**. Тази област се създава **след** списъка с параметри (вж. послесния дотук програмен фрагмент).
