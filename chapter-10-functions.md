...
using namespace std;
void printSentence();  // Деклариране 
int main() {
    printSentence();   // Извикване 
    return 0;
}
void printSentence() {              
...
void printSentence() {                     
    cout << "I am learning functions.";    
} // Деклариране и дефиниция (едновременно)                                        
int main() {
    printSentence();
    return 0;
}
...
Първата ни стъпка е да създадем **`void`** функция за **принтиране на заглавната част** от касовата бележка (header) (под създаване на функция трябва да разбираме писане на **прототип** и **дефиниция** **отделно**). Нека й дадем смислено име, което описва кратко и ясно задачата й, например **`PrintReceiptHeader`**. В тялото й ще напишем кода от примера по-долу:
